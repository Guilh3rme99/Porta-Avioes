...
void Rex::loadFromFile(std::string_view path){
...
void Rex::paintGL(GLint modelMatrixLoc, GLint colorLoc){
  ...
  //Metal gear Rex(cinza)
  model = glm::mat4 {1.0f};
  model = glm::translate(model, glm::vec3(-1.3f, 2.9f, -2.5f));
  model = glm::rotate(model, glm::radians(270.0f), glm::vec3(0, 1, 0));
  model = glm::scale(model, glm::vec3(0.4f));

  abcg::glUniformMatrix4fv(modelMatrixLoc, 1, GL_FALSE, &model[0][0]);
  abcg::glUniform4f(colorLoc, 0.44f, 0.44f, 0.44f, 1.0f);
  abcg::glDrawElements(GL_TRIANGLES, m_indices.size(), GL_UNSIGNED_INT,
                       nullptr);
